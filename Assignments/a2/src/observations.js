...
function observationSummary2(data) {
  data.results.forEach((data1) =>
    console.log(`${data1.id}@${data1.observed_on_details.date}: ${data1.species_guess}`)
  );
}
...
function observationsByPrivacy(data, geoPrivacy) {
  ...
  if (geoPrivacy !== null && geoPrivacy !== 'open' && geoPrivacy !== 'hidden') throw 'Error';
  let arr = [];
  for (let res of data.results) {
    if (res.geoprivacy === geoPrivacy) {
      arr.push(res);
    }
  }
  return arr;
...
function transformObservation(original) {
  // TODO
  let coords = original.location.split(',');
  let photoArr = [];
  let researchbool = false;
  let lat = +coords[0];
  let lng = +coords[1];
  for (let res of original.photos) {
    photoArr.push(res.url);
  }

  if (original.quality_grade === 'research') researchbool = true;
  let newObj = {
    id: original.id,
    speciesGuess: original.species_guess,
    isResearchQuality: researchbool,
    geoCoords: [lng, lat],
    photoUrls: photoArr,
    photosCount: photoArr.length,
...
function transformObservations(data) {
  ...
  for (let res of data.results) {
    newArr.push(transformObservation(res));
  }
  return newArr;
}
...
function transformObservations2(data) {
  // TODO - use map()
  let newArr = data.results.map((res) => transformObservation(res));
...
function getObservationsById(data, ...ids) {
  ...
  let newArr = [];
  if (ids.length === 1) {
    isFound = data.results.find((res) => res.id === ids[0]);
    if (isFound) return isFound;
    return null;
  }
  ids.forEach((id) => {
    isFound = data.results.find((res) => res.id === id);

    if (isFound) newArr.push(isFound);
  });
  return newArr;
}
...
function getObservationsByPositionalAccuracy(data, options = {}) {
  // TODO
  let obs = [];
  if (!(options.equal || options.greaterThan || options.lessThan)) return data.results;
  else if (options.equal && options.greaterThan && options.lessThan) {
    obs = data.results.filter((res) => res.positional_accuracy === options.equal);
  } else if (options.greaterThan && options.lessThan) {
    obs = data.results.filter(
      (res) =>
        res.positional_accuracy < options.lessThan && res.positional_accuracy > options.greaterThan
    );
  } else if (options.greaterThan)
    obs = data.results.filter((res) => res.positional_accuracy > options.greaterThan);
  else if (options.lessThan) {
    obs = data.results.filter((res) => res.positional_accuracy < options.lessThan);
...
function getTaxonPhotos(data) {
  ...
  for (let res of data.results) {
    ...
    if (res.taxon.default_photo !== undefined) {
      let splitUrl = res.taxon.default_photo.url.split('://');
      let splitUrl1 = splitUrl[1].split('/');
      let picUrl = `https://${splitUrl1[0]}/photos/${splitUrl1[2]}/`;
      let picObj = {
        original: picUrl + 'original.jpg',
        square: picUrl + 'square.jpg',
        small: picUrl + 'small.jpg',
        medium: picUrl + 'medium.jpg',
        large: picUrl + 'large.jpg'
      };
...
function getUserStats(data) {
  ...
  for (let res of data.results) {
    obsCount += res.user.observations_count;
    journalsCount += res.user.journal_posts_count;
    speciesCount += res.user.species_count;
    count++;
  }
...
function extractUserLogins(data) {
  ...
  let newArr = [];
  for (let res of data.results) {
    if (!newArr.includes(res.user.login)) {
      newArr.push(res.user.login);
    }
  }
